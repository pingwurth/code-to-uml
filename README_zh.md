...
> 将代码转化为交互式 UML 报告，浏览 100+ 提供中英文内容覆盖的图表示例 — 浏览器优先渲染，零构建工具。
...
2. **图表展示** — 浏览 20+ 种图表类型的 100+ PlantUML 示例，并提供中英文内容覆盖，在浏览器中实时渲染。
3. **AI 代理技能** — 内置技能定义（`SKILL.md`），让 Cursor、Claude Code、Qwen、Codex 等 AI 编码助手自主生成报告。

无框架。无转译器。无 `node_modules`。启动轻量服务器即可使用。
...
2. （可选）设置 `CTU_HOME` 并安装内置 AI 技能：

```bash
node install.js
# 只为某一个工具安装
node install.js codex
# 只打印当前 shell 的 CTU_HOME 命令，不写入 profile
node install.js --print
```

`install.js` 会为 AI 报告生成写入 `CTU_HOME`，并将内置技能复制到支持的代理目录。仅浏览演示不需要执行该步骤。
...
启动服务器后打开 `http://localhost:5401/demo.html` 浏览所有图表示例。使用标签栏切换图表类型，使用语言切换按钮在中英文之间切换。

### 生成报告

1. 将 `.ctu` 数据文件放置在 `data/claude-code-demo-analysis/`（或其他 `data/<report-slug>/` 目录）。
2. 使用 AI 技能（参见 [AI 代理集成](#ai-代理集成)）或手动基于 `cache/_TEMPLATE.html` 创建 HTML 报告。
3. 生成的报告出现在 `cache/` 目录中，可通过 `index.html` 访问。

### .ctu 文件格式

每个 `.ctu` 文件先定义文件级 `Title` 和 `Describe`，然后包含一个或多个重复的 `[Example]`、`[Description]`、`[UML]`、`[Detail]` 分组：
...
│   ├── demo/                # 内置示例（.ctu 文件）
│   ├── _TEMPLATE.ctu        # 新报告模板
│   └── claude-code-demo-analysis/  # 示例：分析报告数据
├── cache/
...
│   └── SKILL.md             # AI 代理技能定义
├── test/                    # Node 测试
├── package.json             # npm 脚本与包元数据
├── serve.sh / serve.bat     # 平台启动脚本
├── CLAUDE.md                # Claude Code 指引
├── AGENTS.md                # 代理文档
└── install.js               # CTU_HOME 与 AI 技能安装脚本
```
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `lang` | `en` \| `zh` | 语言过滤（默认：`zh`） |
| `dir` | string | `data/` 下的数据子目录（默认：`demo`） |

**响应：** 按图表/分类名称分组的 JSON 对象，每个值都是解析后的示例对象数组。
...
| 字段 | 说明 |
|------|------|
| **请求体** | 包含 `source` 字符串的 JSON 对象 |
| **Content-Type** | `application/json` |
| **响应** | 包含 `svg` 字符串的 JSON 对象，或包含 `error` 字符串 |

### `GET /api/cache-html`

返回 `cache/` 中生成的 HTML 报告，供首页索引使用。

### `DELETE /api/cache-html`

删除单个生成报告及其匹配的 `data/<report-slug>/` 目录。

| 字段 | 说明 |
|------|------|
| **请求体** | 包含 `path` 字段的 JSON 对象，例如 `cache/example.html` |

### `DELETE /api/cache-html/all`

删除生成的缓存报告和匹配的数据目录，同时保留模板文件。
...
| 服务根目录 | 运行 `serve.js` 时的当前工作目录 | 当前工作目录 |
| AI 报告根目录 | `CTU_HOME` 环境变量（由 `install.js` 设置） | 安装时的项目根目录 |
| 界面语言 | `localStorage` 键 `plantuml-docs-lang` | `en` |
...
```
用户打开 http://localhost:5401/demo.html
        │
        ▼
    [渲染队列]
        │
