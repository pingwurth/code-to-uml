...
请生成 full Code-To-UML 中文报告，输出到 cache/current-project-analysis.html。
...
请生成 full 英文 Code-To-UML 报告，输出到 cache/auth-module-analysis.html。
...
请生成 compact 中文 Code-To-UML 报告，输出到 cache/normalize-file-analysis.html。
...
请生成 compact 中文报告，输出到 cache/parse-user-input-analysis.html。
...
```text
使用 $code-to-uml，更新 cache/payment-flow-analysis.html 对应的报告。
...
```text
使用 $code-to-uml，修复 cache/order-state-analysis.html 的验证失败。
先复现这个命令：
node skills/code-to-uml/scripts/validate-report.js --root . --html cache/order-state-analysis.html --lang zh --scope module --complexity medium --mode full --strict --render
